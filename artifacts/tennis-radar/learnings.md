...
- **판단**: yeyak `rsv_svc_id`는 사이트가 검색을 JS로 렌더링해서 `curl`로 못 찾음 — 브라우저 조작이 필요해 이번엔 미루고 plan.md 미결정 항목에 기록만 함 (사용자 확인 대기). → **후속**: 사용자가 브라우저로 홍은테니스장 실제 URL 하나를 찾아줘서 망원 대체. 뚝섬·반포는 "크롤링해달라"는 요청으로 다시 조사.
- **JS 렌더링 검색도 결국 폼 뒤엔 평범한 POST 엔드포인트가 있다**: Playwright로 실제 브라우저를 띄우려다(`page.clock`처럼 headless가 필요할 줄 알았음) 홈페이지 HTML 정적 소스에서 검색창의 `onclick` 핸들러(`fnTotSearch()`)를 따라가 보니 `<form>`이 `/web/search/selectPageListTotalSearch.do`로 그냥 POST 제출되고, 그 결과 페이지 안에 또 다른 `fnSvcMore()` 함수가 `/web/search/selectPageListSvcMoreAjax.do`에 `sch_text` 파라미터로 POST해서 JSON(`resultList[].SVC_ID`)을 받아오는 구조였음. 이 ajax 엔드포인트를 `curl -X POST --data-urlencode "sch_text=뚝섬"` 식으로 직접 호출하니 브라우저 없이 검색 결과(진짜 `rsv_svc_id` 목록)를 바로 얻음. 결과: **뚝섬한강공원·반포한강공원은 yeyak에 테니스장으로 등록된 적이 없는 시설**이었다는 걸 확인 — 초기 seed 자체가 존재하지 않는 시설이었음. 실존하는 한강공원 테니스장(잠원·광나루)으로 교체. **일반화 가능, 재발 가능성 높음**: "이 사이트는 JS라서 크롤링하려면 headless 브라우저가 필요하다"고 바로 단정하지 말고, 정적 HTML 안의 폼 action과 onclick 핸들러(JS 함수 정의)를 먼저 따라가 보면 대부분 평범한 form-POST/ajax 엔드포인트가 나온다 — `curl`만으로 훨씬 빠르고 가볍게 끝난다.
- **판단**: 교체한 잠원·광나루 코트의 `rsv_svc_id`는 검색 결과에서 "주말·공휴일" 전용 회차를 선택한 것 — 실제로 코트별·요일유형(평일/주말)별로 서로 다른 `rsv_svc_id`가 발급되는 걸 확인했다(예: 현저테니스장에 평일용/주말용 두 ID 존재). 앱은 코트당 ID 1개만 모델링해서 이 두 코트는 평일 예약 시 링크가 안 맞을 수 있음 — spec 범위 밖으로 판단해 plan.md에 제약사항으로만 기록하고 데이터 모델 확장은 하지 않음.
