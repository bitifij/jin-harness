...
- **구현 대상**:
  ...
  - colocated `.test.tsx`
- **수용 기준**:
  - [x] 위치 허용 시 현재 위치 기준 반경 10km 코트가 거리순으로 렌더된다 (aggregateCourts가 거리순 보장, court-list가 그대로 렌더)
  - [x] 위치 거부 시 여의도 기준으로 렌더되고 "기본 위치(여의도)" 안내 + "현재 위치 사용하기" 버튼이 보인다
  - [x] "현재 위치 사용하기" 클릭 시 geolocation이 재호출되고, 허용되면 실위치 기준으로 목록이 갱신된다 — **범위 축소**: 별도 "수동 위치 입력" UI는 두지 않고 재요청 버튼만 제공 (wireframe에 수동 입력 UI 없음, learnings.md 기록)
  - [x] 반경 5km로 변경 시 밖 코트가 사라지고, 0곳이면 "반경 내 코트가 없습니다" 메시지가 표시된다
...
- **[해결됨] yeyak 3개 코트의 `rsv_svc_id`가 전부 가짜/잘못된 값이었음 — 예약 딥링크가 "페이지를 찾을 수 없습니다" 에러로 연결됨.** 사용자가 "예약하러 가기 연결이 깨져있다"고 제보해 발견 (2026-07-09).
  - `yeyak-mangwon`(망원한강공원) → `yeyak-hongeun`(홍은테니스장, `rsv_svc_id=S260623114555963304`, 사용자가 브라우저로 직접 확인한 값)
  - `yeyak-ddukseom`(뚝섬), `yeyak-banpo`(반포) → **실제로 yeyak에 등록된 시설이 아니었음**. `/web/search/selectPageListSvcMoreAjax.do`(검색폼 `search2`의 `sch_text` 파라미터로 POST하는 실제 ajax 엔드포인트, 브라우저 없이 curl로 재현 가능함을 확인)로 "뚝섬"·"반포"만 검색해보니 농구장·배드민턴장·축구장·촬영장소만 나오고 테니스장은 없었음 → 처음부터 존재하지 않는 시설을 seed 데이터로 넣었던 것으로 추정. 같은 검색으로 실제 존재하는 한강공원 테니스장(`잠원한강공원`·`광나루한강공원`)을 찾아 `yeyak-jamwon`·`yeyak-gwangnaru`로 교체.
  - **새 제약사항**: 교체한 두 코트의 `rsv_svc_id`는 검색 결과에 "평일 주간"·"주말 야간" 등 코트별·요일유형별로 서로 다른 ID가 각각 발급되는 게 확인됨(예: 현저테니스장은 평일용/주말용 ID가 별개). 지금 넣은 ID는 "주말·공휴일" 전용 회차 — 앱은 코트당 `rsv_svc_id` 1개만 모델링하므로 이 두 코트는 평일 예약 시 딥링크가 맞지 않을 수 있음. 코트당 요일유형별 다중 ID 지원은 향후 스코프.
- **[해결됨]** gytennis 딥링크(`/rsvDaily/{id}/{date}`)와 양평누리 딥링크(`rentalSvc/svc/selectSvcReservationCalenderView.do`)가 둘 다 존재하지 않는 잘못된 경로였음 — gytennis는 실제 홈페이지 로그인 페이지로 리다이렉트, 양평누리는 404. 실제 사이트 검증 후 각각 `/daily/{id}/{date}`, `page/rent/s04.od.list.asp`로 수정 완료 (커밋 `14a9d83`)
- gytennis raw HTML의 available 아이콘 판별 셀렉터 — Task 2에서 실제 HTML fixture 확보 후 확정 (파싱 로직 자체는 실제 페이지로 검증 완료, 아래 TLS 항목 참조)
- **[신규, 배포 차단] gytennis.or.kr TLS 인증서 체인 불완전 — Node/Bun `fetch()`가 실호출 시 100% 실패한다.** Task 12 실행 후 수동으로 `/api/courts`를 실제 네트워크로 호출해보니(양평누리·yeyak과 달리) gytennis만 매 코트마다 `loadError=true`. 원인 추적: `curl`은 성공하지만 Bun/Node `fetch()`는 `UNABLE_TO_VERIFY_LEAF_SIGNATURE`로 실패 — 서버가 중간 인증서(Sectigo Public Server Authentication CA DV R36)를 핸드셰이크에 포함하지 않아 브라우저/curl(OS 키체인의 AIA fetching)은 넘어가지만 Node/undici(AIA 미지원)는 체인을 완성 못함. `openssl s_client`로 확인: `Verify return code: 0`이지만 이는 openssl이 로컬 체인을 보충했기 때문. **파서 로직 자체는 정상 — 실제 페이지로 직접 검증해 정확한 슬롯 데이터를 반환함을 확인.** 프로덕션(Vercel 등 Node 런타임)에서도 동일하게 실패할 것이므로 배포 전 반드시 해결 필요. 옵션: (a) gytennis 운영자에게 서버 체인 수정 요청, (b) `NODE_EXTRA_CA_CERTS`로 누락된 중간 인증서를 명시적으로 신뢰 목록에 추가 (특정 도메인 한정, `rejectUnauthorized: false`처럼 전역 검증을 끄는 것과는 다름 — 안전), (c) 별도 프록시/서버리스 함수로 우회. **`rejectUnauthorized: false`처럼 인증서 검증을 통째로 끄는 방식은 채택하지 않는다** (보안 저하).
