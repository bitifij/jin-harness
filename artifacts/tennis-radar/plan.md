...
- **[확인됨, 배포 차단] yeyak 3개 코트의 `rsv_svc_id`가 전부 가짜/잘못된 값 — 예약 딥링크가 "페이지를 찾을 수 없습니다" 에러로 연결됨.** 사용자가 "예약하러 가기 연결이 깨져있다"고 제보해 실제로 `curl`로 확인 (2026-07-09) — 응답은 HTTP 200이지만 본문이 yeyak.seoul.go.kr의 소프트 404 에러 페이지(`페이지 주소가 잘못 입력되었거나 변경되어...`). 사이트가 검색 결과를 JS로 그려서 `curl`로 진짜 ID를 못 찾음 — 브라우저로 직접 뚝섬·반포·망원 한강공원 테니스장을 검색해 URL의 `rsv_svc_id` 값을 확인해야 함. 주간/야간 분리 여부도 이때 같이 확인 (분리 안 되면 시간필터에서 "시간 미상" 유지)
