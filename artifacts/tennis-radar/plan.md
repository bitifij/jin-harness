...
- **[부분 해결] yeyak 3개 코트의 `rsv_svc_id`가 전부 가짜/잘못된 값이었음 — 예약 딥링크가 "페이지를 찾을 수 없습니다" 에러로 연결됨.** 사용자가 "예약하러 가기 연결이 깨져있다"고 제보해 실제로 `curl`로 확인 (2026-07-09). 사이트가 검색 결과를 JS로 그려서 `curl`로는 진짜 ID를 못 찾음 — 브라우저로 직접 찾아야 함.
  - `yeyak-mangwon`(망원한강공원, 가짜 ID) → 사용자가 브라우저로 확인한 **실제 값**으로 교체: `yeyak-hongeun`(홍은테니스장, `rsv_svc_id=S260623114555963304`). 좌표는 페이지 내 legacy 주석에서 추출한 근사값(37.59699, 126.94042) — 실측 권장.
  - `yeyak-ddukseom`(뚝섬), `yeyak-banpo`(반포)는 **여전히 가짜 ID** — 브라우저로 검색해 실제 `rsv_svc_id` 확인 필요 (또는 홍은처럼 검증된 다른 코트로 교체)
  - 주간/야간 분리 여부: 홍은 확인 결과 하나의 `rsv_svc_id`(시설 단위)에 "평일 주간"·"주말 야간" 프로그램이 함께 표시됨 — 별도 ID 분리는 필요 없어 보임 (현재 모델 그대로 유효)
